...
largest Canadian metropolitan areas in 2019 and 2023. The results refer
to all the Dissemination Areas (DA) (the smallest publicly available
geography provided by Statistics Canada
\citep{governmentofcanadaDictionaryCensusPopulation2021a}) within each
metropolitan area, yielding origin-destination pairs containing
information on travel time by public transit, population, and total
employment within those spatial units. These estimates were created by
leveraging expertise in data science, computer programming, and
transportation accessibility using the \{r5r\} package
...
\subsection{Travel time matrices}\label{travel-time-matrices}

The main contents of the \{canaccessR\} package are the travel time
matrices. These matrices (data frames) comprise the estimated travel
time by transit from each origin and destination Dissemination Area
pairs. The matrices also contain identifiers, such as the unique origin
and destination DA codes, region identifier (\emph{e.g.}, ``cal'' for
the region Calgary) and name, the year in which the travel times refers
to, and the date and time of departure for a trip that originates at the
Dissemination Area of origin. Besides these information, the matrices
feature the total population in the origin DA and the total employment
in the destination DA (both according to the 2016 Census). There are two
matrices for each metropolitan area, one containing the travel times to
jobs and the other to groceries stores, making it 20 datasets (the
information from Toronto, Hamilton, and Waterloo are aggregated at the
Greater Golden Horseshoe travel time matrices).

\textbf{HOW TO MAKE THE INFORMATION FROM THE TTM INTO SPATIAL DATA?} The
information can be turned into spatial data using the

Below, we present some descriptive statistics from the travel time
matrices contained in the \{canaccessR\} package.

\subsection{Other contents}\label{other-contents}
