...
The results refer to all the Dissemination Areas (DA) (the smallest publicly available geography provided by Statistics Canada [@governmentofcanadaDictionaryCensusPopulation2021a]) within each metropolitan area, yielding origin-destination pairs containing information on travel time by public transit, population, and total employment within those spatial units.
...
The locations included in the data package comprise the 12 largest (population-wise) Census metropolitan areas (CMA's) based on the 2021 Canadian Census [@governmentofcanada2021CensusPopulation2021] [^2]. 
...
These matrices (data frames) comprise the estimated travel time by transit from each origin and destination Dissemination Area pairs.
The matrices also contain identifiers, such as the unique origin and destination DA codes, region identifier (*e.g.*, "cal" for the region Calgary) and name, the year in which the travel times refers to, and the date and time of departure for a trip that originates at the Dissemination Area of origin.
Besides these information, the matrices feature the total population in the origin DA and the total employment in the destination DA (both according to the 2016 Census).
There are two matrices for each metropolitan area, one containing the travel times to jobs and the other to groceries stores, making it 20 datasets (the information from Toronto, Hamilton, and Waterloo are aggregated at the Greater Golden Horseshoe travel time matrices). 

**HOW TO MAKE THE INFORMATION FROM THE TTM INTO SPATIAL DATA?**
The information can be turned into spatial data using the 

Below, we present some descriptive statistics from the travel time matrices contained in the {canaccessR} package. 
